...
export default function AdminPanelPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/stats/dashboard`);
        if (response.ok) {
          const result = await response.json();
          if (result.success && result.data) {
            const { totalUsers, totalTrips, totalDestinations, totalReviews } = result.data;
            setStats([
              {
                title: "Total Users",
                value: totalUsers.toString(),
                change: "+12.5%", // Demo data for change

...
export default function TripsPage() {
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            const mappedData = result.data.map(item => ({
              id: item._id,
              title: item.title || item.name || "Unknown Trip",
              destination: item.destination || "Unknown Destination",
              traveler: item.traveler || item.user || "Unknown User",
              startDate: new Date(item.startDate || item.createdAt || Date.now()).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
              endDate: new Date(item.endDate || Date.now()).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
              status: item.status || "Upcoming",
              amount: item.amount || 0,
              travelers: item.travelers || 1,
              paymentStatus: item.paymentStatus || "Pending",
              image: item.image || item.thumbnail || "https://images.unsplash.com/photo-1436491865332-7a61a109cc05"
            }));
            setTrips(mappedData);
          }
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/trips/${deleteTrip.id}`, {
        method: "DELETE"
      });
      if (response.ok) {
        setTrips((current) =>
          current.filter((trip) => trip.id !== deleteTrip.id)
        );
        setDeleteTrip(null);
      }
    } catch (error) {
